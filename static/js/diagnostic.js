...
function serverUpdate() {
    ...
    $.ajax({
        ...
        data: {
            ...
            'diagnostic_counter': JSON.stringify(diagnostic_counter)
        },
        dataType: 'json',
    ...
    .then(response => {
        ...
        diagnostic_counter = JSON.parse(response.diagnostic_counter);
    ...
    .then( () => {
        setTimeout(displayDiagnosticCounter, 250)
    });
...
function displayDiagnosticCounter() {
    hideAllElements()
    let diagnostic = document.querySelector(`#diagnostic`);
    diagnostic.innerHTML = 'The counter is at ' + diagnostic_counter
...
document.addEventListener('DOMContentLoaded', function () {
    hideAllElements()
    document.querySelector(`#block_intro`).style.display = 'block';
    ...
    setTimeout( () => {
            ...
            if (key_pressed) {
                let diagnostic = document.querySelector(`#diagnostic`);
                diagnostic.innerHTML = 'Key Pressed at diagnostic_counter' + diagnostic_counter
                diagnostic.style.display = 'block';
            }
        }
        displayDiagnosticCounter()
    }, 500);
